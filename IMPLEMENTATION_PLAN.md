...
- [x] 8.12 — 풍자 톤 한·영 양방향 점검 + messages 키 parity + lint/typecheck/build 모두 exit 0 → `<promise>PROJECT_DONE</promise>` — messages parity 0 asymmetry (737 키 en/ko 동일, Phase 7: 388 → Phase 8: 737, +349). ICU MessageFormat 검증 0 fail (양 locale). lint/typecheck/build 모두 exit 0. 16 SSG 라우트 prerender (8 페이지 × 2 locale: Home / Benchmarks / Capabilities / Architecture / Security / Docs / Careers / About) + 7 OG image 라우트 (Home + 6 페이지 locale-aware). 풍자 키 영문 18곳·한글 21곳 분산 — Docs: "may or may not exist / 존재할 수도 있고 아닌 수도", "가공 CI 봇", "JSON 의견", "큐는 인내심", "PTO 인 내부 팀", "팬픽션", "200 받으셨다면 — frontier model 호출 안 됐습니다". Careers: "60명 600명처럼 출시 6명처럼 토론", "Korean-first English-second footnote-third", "34층은 GPU pre-staging 투어 안 합니다", "Actual photo. Probably / 실제 사진. 아마도", "1시간의 어색한 small talk + 점심 무료", "대체로 무제한 PTO — 다 알고 있습니다 대시보드".
