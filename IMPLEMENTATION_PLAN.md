...
- [ ] Next.js 14+ (App Router, TypeScript) 프로젝트 초기화 — `package.json` / `tsconfig.json` / `next.config.mjs` / `app/layout.tsx` / `app/page.tsx` placeholder + Tailwind CSS 설정 + ESLint(next/core-web-vitals) + `lint`/`typecheck`/`build` npm scripts. 완료 후 AGENTS.md 의 3개 검증 명령이 exit 0
- [ ] shadcn/ui 초기화 + 기본 컴포넌트 (Button, Card, Badge, Separator) install. `components/ui/` 디렉토리 + `lib/utils.ts` (cn 헬퍼) 셋업
- [ ] 추가 의존성 install — `recharts`, `lucide-react`, `next-themes` (다크모드), `voyageai`(SDK), `@vercel/analytics`
